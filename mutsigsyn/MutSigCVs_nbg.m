...
function MutSig_runCV(mutation_file,coverage_file,covariate_file,output_file)
  ...
  for gi=1:ng, if ~mod(gi,1000), fprintf('%d/%d ',gi,ng); end
    ...
    for ni=0:max_neighbors, gidx = ord(ni+1);
      ...
      nfit=nfit+G.n_noncoding(gidx); Nfit=Nfit+G.N_noncoding(gidx);
  ...
  for g=1:ng, if ~mod(g,1000), fprintf('%d/%d ',g,ng); end
    ...
    X = reshape(X_gcp(g,1:ncat,:),ncat,np)';
    % check if the background coverage is bigger than the background mutation number
    cmp = x<=X;
    if ~all(cmp(:)==1)
        badx{end+1} = char(G.gene(g));
        continue
    end
    P0 = hyge2pdf(0,N,x,X);
    P1 = hyge2pdf(1,N,x,X);
  ...
  %write uncalculatable genes
  badx_file = [output_file '.badx.txt'];
  
  writecell(badx,badx_file,'Delimiter','tab')
